...
db.getSyllabuses = (academicSessionId, schoolingProgramId, action) => 
{
    return new Promise((resolve, reject) => 
    {
        try
        {
            ...
            acs.id AS academicSessionId, acs.name AS academicSessionName, COUNT(gws.id) AS isExist
            FROM syllabus s
            JOIN schooling_program sp ON sp.id = s.schooling_program_id
            JOIN academic_session acs ON acs.id = sp.academic_session_id 
            LEFT JOIN grade_wise_syllabus gws ON gws.syllabus_id = s.id`;
            if(parseInt(academicSessionId) > 0)
            {
                sql =sql + ` WHERE acs.id = ${academicSessionId}`;
                if(parseInt(schoolingProgramId) > 0)
                {
                    sql =sql + ` AND s.schooling_program_id = ${schoolingProgramId}`;
                }
                if(action == "Active")
                {
            ...
            else
            {
                if(parseInt(schoolingProgramId) > 0)
                {
                    sql =sql + ` WHERE s.schooling_program_id = ${schoolingProgramId}`;
                    if(action == "Active")
                    {
                        sql =sql + ` AND s.is_active = 1`;
                    }
...
db.duplicateSyllabus = (name, schoolingProgramId) => 
{
    return new Promise((resolve, reject) => 
    {
        try
        {
            let sql = `SELECT s.id AS syllabusId, sp.name AS schoolingProgramName
            FROM syllabus s 
            JOIN schooling_program sp ON sp.id = s.schooling_program_id
            WHERE s.name = '${name}' AND s.schooling_program_id = ${schoolingProgramId}`;
            dbConn.query(sql, (error, result) => 
            {
                if(error)
                {
...
db.insertSyllabus = (syllabus) => 
{
    return new Promise((resolve, reject) => 
    {
        try
        {
            let sql = `INSERT INTO syllabus (name, schooling_program_id, created_on, created_by_id)
            VALUES('${syllabus.name}', ${syllabus.schoolingProgramId}, NOW(), ${syllabus.createdById})`;
...
db.getGradeCategory = (id) => 
{
    return new Promise((resolve, reject) => 
    {
        try
        {
            ...
            FROM grade_category gc WHERE gc.id = ${id}`;
            dbConn.query(sql, (error, result) => 
            {
                if(error)
                {
