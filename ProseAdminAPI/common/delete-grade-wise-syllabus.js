    ...
    try
    {
        ...
        if(reqData.id != undefined )
        {
            if(reqData.id != "")
            {
                ...
                syllabusWiseSubject = await dbCommon.checkGradeWiseSyllabusExist(id);
                if(syllabusWiseSubject.length == 0)
                {
                    let updateGradeWiseSyllabusResult = await dbCommon.deleteGradeWiseSyllabus(id);
                    ...
                    else
                    {
                        ...
                        return res.json({
                            "status_code" : 500,
                            "message" : "Grade Wise Syllabus Already Deleted",
                            "success" : false,
