    ...
    try
    {
        ...
        if(reqData.id != undefined )
        {
            if(reqData.id != "")
            {
                ...
                chapterWiseTopic = await dbCommon.checkInUseSubjectWiseChapterExist(id);
                if(chapterWiseTopic.length == 0)
                {
                    let updateSubjectWiseChapterResult = await dbCommon.deleteSubjectWiseChapter(id);
                    ...
                    else
                    {
                        ...
                        return res.json({
                            "status_code" : 500,
                            "message" : "Subject Wise Chapter Already Deleted",
                            "success" : false,
