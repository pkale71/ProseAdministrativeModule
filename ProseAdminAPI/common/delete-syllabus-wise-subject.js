    ...
    try
    {
        ...
        if(reqData.id != undefined )
        {
            if(reqData.id != "")
            {
                ...
                subjectWiseChapter = await dbCommon.checkInUseSyllabusWiseSubjectExist(id);
                if(subjectWiseChapter.length == 0)
                {
                    let updateSyllabusWiseSubjectResult = await dbCommon.deleteSyllabusWiseSubject(id);
                    ...
                    else
                    {
                        ...
                        return res.json({
                            "status_code" : 500,
                            "message" : "Syllabus Wise Subject Already Deleted",
                            "success" : false,
