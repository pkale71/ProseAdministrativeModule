...
@Component({
  selector: 'app-grade-schedule-setting-add',
  standalone: true,
  imports: [CommonModule, SharedModule],
  templateUrl: './grade-schedule-setting-add.component.html',
  styleUrls: ['./grade-schedule-setting-add.component.scss']
})
export class GradeScheduleSettingAddComponent {
  ...
  gradeForm : FormGroup;
  addGradeScheduleSettingForm : FormGroup;
  isValidForm: boolean;
  saveClicked : boolean;
  schoolUUID : string;
  ...
  ngOnInit() 
  {
    ...
    this.academicYears = [];
    this.addGradeScheduleSettingForm = this.formbuilder.group({
      uuid:[''],
  ...
  async saveGradeScheduleSetting()
  {
    if(!this.saveClicked)
    {
      if(this.addGradeScheduleSettingForm.valid && this.gradeForm.valid && this.academicYearForm.valid)
      {
        this.isValidForm = true;
        this.saveClicked = true;
        
        this.addGradeScheduleSettingForm.controls["grade"].get("id").setValue(this.gradeForm.get("grade").value);
        this.addGradeScheduleSettingForm.controls["academicYear"].get("uuid").setValue(this.academicYearForm.get("academicYear").value);
///////
        try
        {
          let response = await this.gradeScheduleSettingService.createGradeScheduleSetting(this.addGradeScheduleSettingForm.value).toPromise();
          if (response.status_code == 200 && response.message == 'success') 
          {
              this.showNotification("success", "Grade Schedule Setting Created");
