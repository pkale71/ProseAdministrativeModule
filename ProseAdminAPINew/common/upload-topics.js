...
module.exports = require('express').Router().post('/',async(req,res) =>
{
    try
    {
        ...
        if(reqData.applicableFromYear != undefined && reqData.syllabus != undefined && reqData.gradeCategory != undefined && reqData.grade != undefined && reqData.subject != undefined && reqData.chapter != undefined)
        {
            if(JSON.parse(reqData.applicableFromYear)?.id != "" && JSON.parse(reqData.syllabus)?.id != "" && JSON.parse(reqData.gradeCategory)?.id != '' && JSON.parse(reqData.grade)?.id != "" && JSON.parse(reqData.subject)?.id != "" && JSON.parse(reqData.chapter)?.id != "")
            {
                                ...
                                if(subject.length == 1)
                                {
                                    ...
                                    {    
                                        if(req.files.length == 1)
                                        {
                                            ...
                                            {
                                                //get Topics
                                                topics = await dbCommon.getTopics(gradeCategoryId, gradeId, syllabusId, subjectId, chapterId, 'All');                                               
                                                if(topics.length >= 0)
                                                {
                                                    ...
                                                    {  
                                                        ...
                                                        {                  
                                                            let insertTopicResult = await dbCommon.insertMultipleTopic(fileData.data, applicableFromYearId, chapterId, authData.id);
