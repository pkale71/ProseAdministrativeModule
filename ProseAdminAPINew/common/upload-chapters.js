...
module.exports = require('express').Router().post('/',async(req,res) =>
{
    try
    {
        ...
        if(reqData.gradeCategory != undefined && reqData.grade != undefined && reqData.syllabus != undefined && reqData.subject != undefined && reqData.applicableFromYear != undefined)
        { 
            if(JSON.parse(reqData.gradeCategory)?.id != '' && JSON.parse(reqData.grade)?.id != "" && JSON.parse(reqData.syllabus)?.id != "" && JSON.parse(reqData.subject)?.id != "" && JSON.parse(reqData.applicableFromYear)?.id != "")
            {
                ...
                applicableFromYearId = commonFunction.validateNumber(JSON.parse(reqData.applicableFromYear)?.id);

                //check applicable from year
                            ...
                            if(syllabus.length == 1)
                            {
                                ...
                                if(subject.length == 1)
                                {
                                    if(req.files.length == 1)
                                    {
                                        ...
                                        {
                                            //get chapters
                                            chapters = await dbCommon.getChapters(gradeCategoryId, gradeId, syllabusId, subjectId, 'All');                                               
                                            if(chapters.length >= 0)
                                            {
                                                let fileData = readFileAndFormatCheck(req.files[0], chapters, req, res);
                                                if(fileData != "")
                                                {  
                                                    if(fileData.data.length > 0)
                                                    {                  
                                                        let insertChapterResult = await dbCommon.insertMultipleChapter(fileData.data, applicableFromYearId, subjectId, authData.id);
                                                        if(insertChapterResult.insertId > 0)
                                                        {
                                                            //Remove Files
...
function readFileAndFormatCheck(file, chapters, req, res)
{
    try 
    {
        let cellDatas = [];
        ...
        if(sheetName == "Chapter")
        {
            ...
            if (data.length > 0) 
            {
                ...
                if(firstRow[0].toString().trim().toLowerCase() == "chapter name")
                {
                    for (let R = range.s.r + 1; R <= range.e.r; R++) 
                    {
                        ...
                        if (worksheet[cellRef]) 
                        {
                            ...
                            if(cellValue != null && cellValue != "" && cellValue != undefined)
                            {
                                ...
                                let duplicateChapter = chapters.filter(chapter => (chapter.name).toString().toLowerCase() == cellValue.toString().trim().toLowerCase());
                                if(duplicateChapter.length == 0)
                                {
