...
export class NavContentComponent implements OnInit, AfterViewInit {
  ...
  constructor(public nav: NavigationItem, private zone: NgZone, private location: Location, 
    ...
    for(let i=0;i<this.navigation.length;i++)
    {
      ...
      { 
        innerChildren1[k].hidden = this.checkMenuUserCodeByLoginUser(innerChildren1[k].userGradeCode);
   ///Param Setting     
  ...
  /////
        ...
        if(innerChildren1[k].children != undefined)
        {
          ...
          if(innerChildren2 != undefined)
          {
            ...
            {
              innerChildren2[l].hidden = this.checkMenuUserCodeByLoginUser(innerChildren2[l].userGradeCode);
            }
          }
        }
      }
  ...
  checkMenuUserCodeByLoginUser(userGradeCode : string[])
  {
    let result : boolean = true;
    if(userGradeCode)
    {
      for(let i=0;i<userGradeCode.length;i++)
      {
        if(userGradeCode[i] == this.loginUser.userGrade?.code)
        {
          result = false;
          break;
        }
