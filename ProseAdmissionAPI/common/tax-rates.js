...
module.exports = require('express').Router().get('/?*', async(req,res) =>
{
    try
    {
        ...
        {
            academicSessionId = commonFunction.validateNumber(tempParams[0]);
            taxTypeId = commonFunction.validateNumber(tempParams[1]);
        }
        ...
        taxRates = await dbCommon.getTaxRates(academicSessionId, taxTypeId, action);
        if(taxRates.length >= 0)
        {
            res.status(200)
            return res.json({
                "taxRates" : buildJSON.taxRates(taxRates),
                "status_code" : 200,
                "success" : true,                            
